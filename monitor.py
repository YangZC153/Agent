...
SCREENING_CANDIDATES_PER_TOPIC = 10
SCREENING_CANDIDATE_LIMIT = 50
RECENT_PAPER_DAYS = 7
...
2. relevance_score 为 0-100，衡量论文对对应主题的相关性。
3. 代码开源判断只能依据给出的 abstract：
   - 摘要明确写明代码/实现已公开、将公开，或给出 GitHub/GitLab 等仓库链接，code_open_source="是"。
   - 否则 code_open_source="摘要未说明"。不得根据常识或标题猜测。
4. code_url 只能填写摘要中真实出现的代码仓库 URL；没有则为空字符串。
5. 同主题内，代码明确开源的高质量论文应获得更高优先级。
6. 只返回 JSON，不要 Markdown，不要解释。必须为：
...
""".strip()
    ...
    for paper in candidates:
        ...
        try:
            ...
            continue
        if topic_id not in topics_by_id or score < 50:
            continue
        code_status = (
    ...
    print(
        f"[INFO] DeepSeek screened {len(candidates)} candidates; "
        f"kept={len(screened)}"
    )
    return screened
