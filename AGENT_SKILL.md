...
The daily output should send the arXiv paper report first, followed by a
separate four-line investment advice message generated by
`python3 -m investment_advice.dca`.
Investment advice is Feishu-only and must not be written to
`viewer/papers_data.json` or published to GitHub Pages.
...
6. The cron flow includes the second investment advice message after the arXiv report.
...
- the generated prompt includes `python3 -m investment_advice.dca`
- the generated prompt keeps investment advice out of `viewer/papers_data.json` and GitHub Pages
